...
const login = asyncHandle(async (req, res, next) => {
  ...
  if (!matchUser.isActive) {
    return next(
      new ErrorResponse("User is not activated or User has been banned", 400),
    );
  }
...
const changePassword = asyncHandle(async (req, res, next) => {
  ...
  if (newPassword !== confirmNewPassword) {
    return next(
      new ErrorResponse("New password not match with confirm password", 400),
    );
  }
...
const resetPassword = asyncHandle(async (req, res, next) => {
  ...
  if (resetPassword !== confirmResetPassword) {
    return next(
      new ErrorResponse("Reset password not match with confirm password", 400),
    );
  }
